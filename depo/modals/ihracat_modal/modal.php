...
if ($islem == "ihracat_projesi_olsutur_modal") {
    ?>
    <style>
        #new_work_emri_modal {
            overflow-y: scroll;
    ...
    <div class="modal fade" id="new_work_emri_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 100%; max-width: 100%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
                    <button type="button" class="btn-close btn-close-white"
                            id="new_work_emri_modal_kapat"
                ...
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-danger btn-sm" id="new_work_emri_modal_kapat"><i
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#new_work_emri_modal").modal("show");
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res == 1) {
                            ...
                            $("#new_work_emri_modal").modal("hide");
        ...
        $("body").off("click", "#new_work_emri_modal_kapat").on("click", "#new_work_emri_modal_kapat", function () {
            $("#new_work_emri_modal").modal("hide");
        });
...
if ($islem == "kayitli_siparisleri_getir_modal") {
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.get("depo/controller/ihracat_controller/sql.php?islem=ihracat_siparislerini_getir_sql", {cari_id: "<?=$_GET["cari_id"]?>"}, function (res) {
                if (res != 2) {
                    var json = JSON.parse(res);
...
if ($islem == "hizmetleri_getir_sql") {
    ?>
    <div class="modal fade" data-backdrop="static" id="secilecek_detayli_hizmetleri_getir_modal"
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#secilecek_detayli_hizmetleri_getir_modal").modal("show");
            ...
            $("body").off("click", ".guzergah_select").on("click", ".guzergah_select", function (event) {
                var id = $(this).attr("data-id");
                let guzergah_adi = $(this).find("td").eq(0).text();
                let konteyner_sayisi = "<?=$_GET["konteyner_sayisi"]?>";
                let birim_id = $(this).attr("birim_id");
                let basilacak_select = "<select style='width: 50% !important;' class='custom-select custom-select-sm birim_id'>";
                ...
                $("#secilecek_detayli_hizmetleri_getir_modal").modal("hide");
...
if ($islem == "kayitli_ihracat_is_emirlerini_getir_modal") {
    ?>
    <div class="modal fade" data-backdrop="static" id="kayitli_is_emirleri_modal"
         data-bs-keyboard="false" role="dialog">
        <div class="modal-dialog"
             style="width: 35%; max-width: 35%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">KAYITLI İŞ EMİRLERİ
                    <button type="button" class="btn-close btn-close-white" id="modal_kapat1"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="col-md-12 row">
                        <table class="table table-sm table-bordered w-100 display nowrap"
                               style="cursor:pointer;font-size: 13px;"
                               id="depo_is_emirleri_kayitli_receteler">
                            <thead>
                            <tr>
                                <th id="click1_stok">İş Emri Adı</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#kayitli_is_emirleri_modal").modal("show");

            setTimeout(function () {
                $("#click1_stok").trigger("click");
            }, 500);

            var stok_table = $('#depo_is_emirleri_kayitli_receteler').DataTable({
                scrollY: '35vh',
                scrollX: true,
                "info": false,
                paging: false,
                ...
                createdRow: function (row) {
                    $(row).find("td").eq(0).css("text-align", "left");
                    $(row).find("td").eq(1).css("text-align", "left");
                    $(row).addClass("secilen_is_emri_taslagi");
                },
                "rowCallback": function (row, data) {
                    $(row).attr("data-id", data.id);
                }
            })
            $.get("depo/controller/ihracat_controller/sql.php?islem=kayitli_is_emirleri_sql", function (result) {
                if (result != 2) {
                    var json = JSON.parse(result);
                    var basiacak_arr = [];
                    json.forEach(function (item) {
                        let newRow = {
                            is_emri_adi: item.is_emri_adi,
                            id: item.id
                        }
                        basiacak_arr.push(newRow);
                    });
        ...
        $("body").off("click", ".secilen_is_emri_taslagi").on("click", ".secilen_is_emri_taslagi", function () {
            let id = $(this).attr("data-id");
            $.get("depo/controller/ihracat_controller/sql.php?islem=secilen_is_emri_getir_sql", {id: id}, function (res) {
                if (res != 2) {
                    var json = JSON.parse(res);
                    json.forEach(function (item) {
                        let row = is_emri_kalemler_table.row.add([item.stok_adi, item.birim_adi, item.sefer_sayisi, item.islem]).draw(false).node();
                        $(row).attr("hizmet_id", item.stok_id);
                        $(row).attr("birim_id", item.birim_id);
                    });
                    $("#kayitli_is_emirleri_modal").modal("hide");
                }
            })
        });
...
if ($islem == "depo_is_emri_guncelle_modal") {
    ?>
    <style>
        #new_work_emri_guncelle_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="new_work_emri_guncelle_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 100%; max-width: 100%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
                    <button type="button" class="btn-close btn-close-white"
                            id="new_work_emri_guncelle_modal_kapat"
                            data-bss-dissmiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="page-content fade-in-up">
                    <div class="ibox">
                            ...
                            <div class="col-12 row">
                                <div class="col-md-7">
                                    ...
                                    <div class="col-12 row no-gutters">
                                        ...
                                        <div class="col mx-2">
                                            <button class="btn btn-success btn-sm mt-4" id="depo_ihracat_urun_guncelle">
                                                <i
                                                        class="fa fa-plus"></i> Ekle
                                            </button>
                                        </div>
                                ...
                                <div class="col-md-5">
                                    <button class="btn btn-primary btn-sm" id="hizmet_ekle"><i
                                                class="fa fa-check"></i> HİZMET SEÇ
                                    </button>
                                    <button class="btn btn-secondary btn-sm" id="kayitli_is"><i
                                                class="fa fa-file"></i> KAYITLI İŞ
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-danger btn-sm" id="new_work_emri_guncelle_modal_kapat"><i
                                        class="fa fa-close"></i> Vazgeç
                            </button>
                            <button class="btn btn-success btn-sm" id="is_emri_guncelle_button"><i
                                        class="fa fa-plus"></i>
                                Kaydet
                            </button>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#new_work_emri_guncelle_modal").modal("show");
        ...
        $.get("depo/controller/ihracat_controller/sql.php?islem=is_emri_ana_bilgileri_getir_sql", {id: "<?=$_GET["id"]?>"}, function (res) {
            if (res != 2) {
                var item = JSON.parse(res);
                $("#firma_kodu").val(item.cari_kodu);
                $("#firma_kodu").attr("data-id", item.cari_id);
                ...
                $.get("depo/controller/ihracat_controller/sql.php?islem=siparisin_urunlerini_getir_sql", {siparis_id: item.siparis_id}, function (respone) {
                    if (res != 2) {
        ...
        $("body").off("focusout", ".sefer_sayisini_degistir_input").on("focusout", ".sefer_sayisini_degistir_input", function () {
            let id = $(this).attr("data-id");
            let sefer_sayisi = $(this).val();
            $.ajax({
                url: "depo/controller/ihracat_controller/sql.php?islem=sefer_sayisi_degisikligi_sql",
                type: "POST",
                data: {
                    id: id,
                    sefer_sayisi: sefer_sayisi
                ...
                success: function (res) {
                    if (res == 1) {
                        ...
                        Toast.fire({
                            icon: "success",
                            title: "Sefer Sayısı Güncellendi"
                        });
                    }
                }
            });
        ...
        $("body").off("click", "#is_emri_guncelle_button").on("click", "#is_emri_guncelle_button", function () {
            let gonderilecek_siparis_arr = [];
            $(".depo_is_emir_siparisten_gelen_kalemler").each(function () {
                let hizmet_id = $(this).attr("hizmet_id");
                let fabrika_ref = $(this).find("td").eq(1).text();
            ...
            $(".alis_urun_list").each(function () {
                ...
                let birim_id = $(this).find("td:eq(1) option:selected").val();
                let sefer_sayisi = $(this).find("td:eq(2) input").val();
                let id = $(this).find("td:eq(3) button").attr("data-id");
            ...
            if (cari_id == undefined || cari_id == "") {
                ...
                $.ajax({
                    url: "depo/controller/ihracat_controller/sql.php?islem=is_emri_olustur_ve_guncelle_sql",
                    type: "POST",
                    data: {
                        cari_id: cari_id,
                        siparis_tarihi: siparis_tarihi,
                    ...
                    success: function (res) {
                        if (res == 1) {
                            Swal.fire(
                                "Başarılı!",
                                "İş Emri Başarı İle Güncellendi",
                                "success"
                            );
                            $("#new_work_emri_guncelle_modal").modal("hide");
                            $.get("depo/view/depo_konteyner_giris.php", function (getList) {
                                $(".admin-modal-icerik").html("");
                                $(".admin-modal-icerik").html(getList);
                            });
        ...
        $.get("depo/controller/ihracat_controller/sql.php?islem=is_emri_urunlerini_getir_sql", {id: "<?=$_GET["id"]?>"}, function (result) {
            if (result != 2) {
                var json = JSON.parse(result);
                json.forEach(function (item) {
                    is_emri_kalemler_guncelle_table.row.add([item.stok_adi, item.birim_adi, item.sefer_sayisi, item.islem]).draw(false);
                });
            }
        })
        ...
        $("body").off("click", ".is_emri_kalem_urununu_sil_button").on("click", ".is_emri_kalem_urununu_sil_button", function () {
            let id = $(this).attr("data-id");
            let row = $(this).closest("tr");
            $.ajax({
                url: "depo/controller/ihracat_controller/sql.php?islem=is_emri_urunu_sil_sql",
                type: "POST",
                data: {
                    id: id
                },
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res != 1) {
                        let id = res.split(":");
                        id = id[1];
                        is_emri_guncelle_table.row.add([hizmet_adi, fabrika_ref, ep_ref, mal_kodu, mal_adi, konteyner_sayisi, konteyner_tipi, yuk_miktari, birim_adi, "<button class='btn btn-danger btn-sm listedeki_urunu_sil_button' data-id='" + id + "'><i class='fa fa-trash'></i></button>"]).draw(false);
                    } else {
                        Swal.fire(
                            "Oops...",
                            "Bilinmeyen Bir Hata Oluştu",
        ...
        $("body").off("click", ".depo_is_emir_siparisten_gelen_kalemler").on("click", ".depo_is_emir_siparisten_gelen_kalemler", function () {
            $("#kayitli_is").prop("disabled", false);
            $("#hizmet_ekle").prop("disabled", false);
            $(".depo_is_emir_siparisten_gelen_kalemler").removeClass("selected");
            $(".depo_is_emir_siparisten_gelen_kalemler").removeClass("secildi");
            $(this).addClass("selected");
            $(this).addClass("secildi");
        ...
        $("body").off("click", "#hizmet_ekle").on("click", "#hizmet_ekle", function () {
            let konteyner_sayisi = 0;
            let konteyner_tipi = 0;
            $(".depo_is_emir_siparisten_gelen_kalemler").each(function () {
                if ($(this).hasClass("secildi")) {
        ...
        $("body").off("click", "#new_work_emri_guncelle_modal_kapat").on("click", "#new_work_emri_guncelle_modal_kapat", function () {
            $("#new_work_emri_guncelle_modal").modal("hide");
        });
