...
if ($islem == "depo_siparis_olustur_modal_getir") {
    ?>
    <style>
        #depo_ithalat_siparis_olustur_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="depo_ithalat_siparis_olustur_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 75%; max-width: 75%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
                    <button type="button" class="btn-close btn-close-white"
                            id="depo_ithalat_siparis_olustur_modal_kapat"
                ...
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        <div class="ibox-head" style='background-color:#9DB2BF'>
                            <div class="ibox-title" style='color:white; font-weight:bold;'>DEPO İTHALAT SİPARİŞİ</div>
                        </div>
                            ...
                            <div class="col-12 row">
                                ...
                                <div class="col-6">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7">
                                            <input type="date" id="demoraj_tarihi" class="form-control form-control-sm"
                                                   value="<?= date("Y-m-d") ?>">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-danger btn-sm" id="depo_ithalat_siparis_olustur_modal_kapat"><i
                                        class="fa fa-close"></i> Vazgeç
                            </button>
                            <button class="btn btn-success btn-sm" id="depo_ithalat_siparisi_kaydet_button"><i
    ...
    <script>
        ...
        $(document).ready(function () {
            $.get("depo/controller/ithalat_controller/sql.php?islem=son_epro_referans_sql", function (res) {
                if (res != 2) {
            ...
            $("#depo_ithalat_siparis_olustur_modal").modal("show");
            setTimeout(function () {
        ...
        $("body").off("click", "#depo_ithalat_siparis_olustur_modal_kapat").on("click", "#depo_ithalat_siparis_olustur_modal_kapat", function () {
            $("#depo_ithalat_siparis_olustur_modal").modal("hide");
        });
        ...
        $("body").off("click", "#depo_ithalat_siparisi_kaydet_button").on("click", "#depo_ithalat_siparisi_kaydet_button", function () {
            ...
            if (cari_id == undefined || cari_id == "") {
                ...
                $.ajax({
                    url: "depo/controller/ithalat_controller/sql.php?islem=depo_ithalat_konteyner_siparis_kaydet_sql",
                    type: "POST",
                    data: {
                        cari: cari_id,
...
if ($islem == "siparisi_guncelle_modal_getir") {
    ?>
    <style>
        #depo_ithalat_siparis_guncelle_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="depo_ithalat_siparis_guncelle_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 75%; max-width: 75%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
                    <button type="button" class="btn-close btn-close-white"
                            id="depo_ithalat_siparis_guncelle_modal_kapat"
                            data-bss-dissmiss="modal"
                            aria-label="Close"></button>
                </div>
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-danger btn-sm" id="depo_ithalat_siparis_guncelle_modal_kapat"><i
                                        class="fa fa-close"></i> Vazgeç
                            </button>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#depo_ithalat_siparis_guncelle_modal").modal("show");
            setTimeout(function () {
                $("#click12").trigger("click");
            }, 500);
            siparis_table = $('#siparis_table_list').DataTable({
            ...
            $.get("depo/controller/ithalat_controller/sql.php?islem=guncellenecek_siparis_ana_bilgi", {id: "<?=$_GET["id"]?>"}, function (res) {
                if (res != 2) {
                    var item = JSON.parse(res);
                    ...
                    $("#cut_off_tarihi").val(demoraj_tarihi[0]);
        ...
        $.get("depo/controller/ithalat_controller/sql.php?islem=siparis_ihracat_urun_bilgisi_sql", {id: "<?=$_GET["id"]?>"}, function (res) {
            if (res != 2) {
                var json = JSON.parse(res);
                json.forEach(function (item) {
        ...
        $("body").off("click", "#depo_ithalat_siparis_guncelle_modal_kapat").on("click", "#depo_ithalat_siparis_guncelle_modal_kapat", function () {
            $("#depo_ithalat_siparis_guncelle_modal").modal("hide");
        });
        ...
        $("body").off("click", ".ithalat_listesindeki_urunu_sil").on("click", ".ithalat_listesindeki_urunu_sil", function () {
            let id = $(this).attr("data-id");
            let row = $(this).closest("tr");
            $.ajax({
                url: "depo/controller/ithalat_controller/sql.php?islem=ithalat_siparis_urunu_sil_sql",
                type: "POST",
                data: {
                    id: id
                },
        ...
        $("body").off("click", "#depo_ihracat_urun_ekle").on("click", "#depo_ihracat_urun_ekle", function () {
            ...
            let birim_adi = $("#birim_id option:selected").text();
            $.ajax({
                url: "depo/controller/ithalat_controller/sql.php?islem=siparise_urun_ekle_sql",
                type: "POST",
                data: {
                    hizmet_id: hizmet_id,
                    fabrika_ref: fabrika_ref,
                ...
                success: function (res) {
                    if (res != 1) {
                        let id = res.split(":");
                        id = id[1];
                        siparis_table.row.add([hizmet_adi, fabrika_ref, ep_ref, mal_kodu, mal_adi, konteyner_sayisi, konteyner_tipi, yuk_miktari, birim_adi, "<button class='btn btn-danger btn-sm ithalat_listesindeki_urunu_sil' data-id='" + id + "'><i class='fa fa-trash'></i></button>"]).draw(false);
                    } else {
                        Swal.fire(
                            "Oops...",
                            "Bilinmeyen Bir Hata Oluştu",
        ...
        $("body").off("click", "#depo_ihracat_siparisi_kaydet_button").on("click", "#depo_ihracat_siparisi_kaydet_button", function () {
            ...
            if (cari_id == undefined || cari_id == "") {
                ...
                $.ajax({
                    url: "depo/controller/ithalat_controller/sql.php?islem=depo_konteyner_siparis_guncelle_sql",
                    type: "POST",
                    data: {
                        cari_id: cari_id,
                        siparis_tarihi: siparis_tarihi,
