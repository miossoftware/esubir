<?php
$islem = $_GET["islem"];
if ($islem == "alt_grup_guncelle_modal") {
    ?>
            ...
            <div class="page-content fade-in-up">
                <div class="ibox">
                    <div class="ibox-head" style='background-color:#9DB2BF'>
                        <div class="ibox-title" style='color:white; font-weight:bold;'>ALT GRUP GUNCELLE</div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-success btn-sm" id="altgrup_ekle_button">Alt Grup Güncelle</button>
                </div>
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function () {
            ...
            $.get("controller/altgrup_controller/sql.php?islem=alt_grup_bilgileri_getir", {id: "<?=$_GET["id"]?>"}, function (result) {
                if (result != 2) {
                    var item = JSON.parse(result);
                    setTimeout(function () {
                        $("#ana_grupid").val(item.ana_grupid);
                        $("#altgrup_adi").val(item.altgrup_adi);
        ...
        $("body").off("click", "#altgrup_ekle_button").on("click", "#altgrup_ekle_button", function () {
            var ana_grupid = $("#ana_grupid").val();
            var altgrup_adi = $("#altgrup_adi").val();
            var aciklama = $("#aciklama").val();
            $.ajax({
                url: "controller/altgrup_controller/sql.php?islem=alt_grup_guncelle_sql",
                type: "POST",
                data: {
