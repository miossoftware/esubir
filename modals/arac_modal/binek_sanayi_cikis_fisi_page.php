...
$islem = $_GET["islem"];
if ($islem == "sanayi_fisi_guncelle_modal_getir") {
    ?>
    <style>
        #sanayi_cikis_fisi_guncelle_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="sanayi_cikis_fisi_guncelle_modal" data-bs-keyboard="false" data-backdrop="static"
         role="dialog">
    ...
    <script>
        ...
        $("body").off("click", "#sanayi_fisi_kaydet").on("click", "#sanayi_fisi_kaydet", function () {
            ...
            if (rowCount == 0) {
                ...
                $.ajax({
                    url: "controller/arac_controller/sql.php?islem=sanayi_fisi_guncelle_sql",
                    type: "POST",
                    data: {
                        cari_id: cari_id,
                        fis_no: fis_no,
                    ...
                    success: function (response) {
                        if (response == 1) {
                            Swal.fire(
                                "Başarılı",
                                "Sanayi Fişi Güncellendi",
                                "success"
                            );
                            $("#sanayi_cikis_fisi_guncelle_modal").modal("hide");
        ...
        $("body").off("click", "#yeni_sanayi_fisi_ekle").on("click", "#yeni_sanayi_fisi_ekle", function () {
            ...
            let gt = toplam_tutar.replace(/\./g, "").replace(",", ".");
            gt = parseFloat(gt);
            let plaka_id = $("#plaka_no_input").attr("data-id");
            let kdv_tutar = $("#kdv_sanayi").attr("kdv_tutar");
            let kt = kdv_tutar.replace(/\./g, "").replace(",", ".");
            kt = parseFloat(kt);
            let kdv_yuzde = $("#kdv_sanayi").val();
            let ky = kdv_yuzde.replace(/\./g, "").replace(",", ".");
            ky = parseFloat(ky);
            let iskonto_tutar = $("#sanayi_iskonto_yuzde").attr("iskonto_tutar");
            let it = iskonto_tutar.replace(/\./g, "").replace(",", ".");
            it = parseFloat(it);
            let iskonto_yuzde = $("#sanayi_iskonto_yuzde").val();
            let iy = iskonto_yuzde.replace(/\./g, "").replace(",", ".");
            iy = parseFloat(iy);
            ...
            if (plaka_id == undefined || plaka_id == "") {
                ...
                $.ajax({
                    url: "controller/arac_controller/sql.php?islem=sanayi_fisi_ekle_sql",
                    type: "POST",
                    data: {
                        plaka_id: plaka_id,
                        fis_id: "<?=$_GET["id"]?>",
                    ...
                    success: function (result) {
                        if (result != 2) {
                            ...
                            oz_mal_servis_fisi.row.add([plaka_adi, plaka_cari, servis_tipi, miktar, birim_adi, birim_fiyat, kdv_yuzde, kdv_tutar, iskonto_yuzde, iskonto_tutar, ara_toplam, toplam_tutar, aciklama, "<button class='btn btn-danger btn-sm binek_sanayi_fisi_sil' data-id='" + id + "'><i class='fa fa-trash'></i></button>"]).draw(false)
        ...
        $("body").off("click", ".binek_sanayi_fisi_sil").on("click", ".binek_sanayi_fisi_sil", function () {
            ...
            $.ajax({
                url: "controller/arac_controller/sql.php?islem=sanayi_fisi_sil_list_sql",
                type: "POST",
                data: {
                    id: id
                },
        ...
        $(document).ready(function () {
            $("#sanayi_cikis_fisi_guncelle_modal").modal("show");

            $.get("controller/arac_controller/sql.php?islem=guncellenecek_yakit_fisi_bilgileri_sql", {id: "<?=$_GET["id"]?>"}, function (response) {
                if (response != 2) {
                    var item = JSON.parse(response);
                    $("#cari_kodu").val(item.cari_kodu);
                    $("#cari_kodu").attr("data-id", item.cari_id);
                    ...
                    tarih = tarih.split(" ");
                    $("#tarih").val(tarih[0])
                    $("#sanayi_aciklama").val(item.aciklama);
                    ...
                    let kdv_toplam = item.kdv_toplam;
                    ...
                    $(".sanayi_ara_toplam_bas").html("");
                    $(".sanayi_ara_toplam_bas").html(ara_toplam + " TL");
                    $(".sanayi_kdv_toplam_bas").html("");
                    $(".sanayi_kdv_toplam_bas").html(kdv_toplam + " TL");
                    $(".sanayi_iskonto_toplam_bas").html("");
                    $(".sanayi_iskonto_toplam_bas").html(iskonto_toplam + " TL");
                    $(".sanayi_genel_toplam_bas").html("");
                    $(".sanayi_genel_toplam_bas").html(genel_toplam + " TL");

                }
            })

            $.get("controller/arac_controller/sql.php?islem=sanayi_fisi_urunlerini_getir_sql", {fis_id: "<?=$_GET["id"]?>"}, function (response) {
                if (response != 2) {
                    ...
                    json.forEach(function (item) {
                        ...
                        let table = oz_mal_servis_fisi.row.add([item.arac_plaka, "", item.servis_tipi, miktar, item.birim_adi, birim_fiyat, kdv_yuzde, kdv_tutar, iskonto_yuzde, iskonto_tutar, ara_toplam, genel_toplam, item.aciklama, "<button class='btn btn-danger btn-sm binek_sanayi_fisi_sil' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"]).draw(false).node();
                        $(table).attr("data-id", item.plaka_id);
                        $(table).attr("birim_id", item.birim_id);
                    });
                }
            })
