...
if ($islem == "yeni_doviz_alim_yap_modal") {
    ?>
    <div class="modal fade" id="doviz_alim_yap_main_modal" data-bs-keyboard="false"
         role="dialog">
                ...
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        <div class="ibox-head" style='background-color:#9DB2BF'>
                            <div class="ibox-title" style='color:white; font-weight:bold;'>DÖVİZ ALIM</div>
                        </div>
                            ...
                            <div class="col-12 row">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7">
                                            <select class="custom-select custom-select-sm" id="tl_hesaplari">
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-success btn-sm" id="doviz_alim_kaydet"><i class="fa fa-check"></i>
                                Kaydet
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#doviz_alim_yap_main_modal").modal("show");

            $.get("controller/banka_controller/sql.php?islem=tl_hesaplari_getir_sql", function (res) {
                if (res != 2) {
        ...
        $("body").off("click", "#doviz_alim_kaydet").on("click", "#doviz_alim_kaydet", function () {
            let fis_no = $("#fis_no").val();
            let tarih = $("#tarih").val();
            let tl_hesaplari = $("#tl_hesaplari").val();
            ...
            if (tl_hesaplari == "") {
                ...
                $.ajax({
                    url: "controller/banka_controller/sql.php?islem=doviz_alim_kaydet_sql",
                    type: "POST",
                    data: {
                        fis_no: fis_no,
                    ...
                    success: function (res) {
                        if (res == 1) {
                            ...
                            $("#doviz_alim_yap_main_modal").modal("hide");
        ...
        $("body").off("change", "#tl_hesaplari").on("change", "#tl_hesaplari", function () {
            let banka_adi = $("option:selected", this).text();
            $.get("controller/banka_controller/sql.php?islem=bankaya_ait_doviz_hesaplari_sql", {banka_adi: banka_adi}, function (res) {
                if (res != 2) {
                    var json = JSON.parse(res);
                    $("#doviz_hesaplari").html("");
                    $("#doviz_hesaplari").append("<option value=''>Bir Döviz Hesabı Seçiniz...</option>");
                    json.forEach(function (item) {
                        $("#doviz_hesaplari").append("" +
                            "<option doviz_turu='" + item.doviz_tipi + "' value='" + item.id + "'>" + item.hesap_adi + "</option>" +
                            "");
                    })
                }
            })
        ...
        $("body").off("change", "#doviz_hesaplari").on("change", "#doviz_hesaplari", function () {
            let val = $("option:selected", this).attr("doviz_turu");
            $("#doviz_turu").val(val);
            if (val == "TL") {
                $("#doviz_kuru_irsaliye").val("1.00");
            } else if (val == "EURO") {
...
if ($islem == "doviz_alim_guncelle_modal") {
    ?>
    <div class="modal fade" id="doviz_alim_guncelle_main_modal" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 50%; max-width: 50%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
    ...
    <script>
        ...
        $(document).ready(function () {
            $("#doviz_alim_guncelle_main_modal").modal("show");
            ...
            $.get("controller/banka_controller/sql.php?islem=bankaya_ait_doviz_hesaplari_sql", function (res) {
                if (res != 2) {
                    var json = JSON.parse(res);
                    $("#doviz_hesaplari").html("");
                    $("#doviz_hesaplari").append("<option value=''>Bir Döviz Hesabı Seçiniz...</option>");
            ...
            $.get("controller/banka_controller/sql.php?islem=banka_alim_bilgileri_sql", {id: "<?=$_GET["id"]?>"}, function (res) {
                if (res != 2) {
                    ...
                    setTimeout(function () {
                        ...
                        $("#tl_hesaplari").val(item.tl_hesap);
                    }, 500);
        ...
        $("body").off("click", "#doviz_alim_kaydet").on("click", "#doviz_alim_kaydet", function () {
            ...
            if (tl_hesaplari == "") {
                ...
                $.ajax({
                    url: "controller/banka_controller/sql.php?islem=doviz_alim_guncelle_sql",
                    type: "POST",
                    data: {
                        fis_no: fis_no,
                        tl_hesap: tl_hesaplari,
                    ...
                    success: function (res) {
                        if (res == 1) {
                            Swal.fire(
                                "Başarılı",
                                "Döviz Alım Güncellendi",
                                "success"
                            )
                            $("#doviz_alim_guncelle_main_modal").modal("hide");
                            $.get("view/doviz_alim.php", function (getList) {
                                $(".admin-modal-icerik").html("");
                                $(".admin-modal-icerik").html(getList);
                            });
