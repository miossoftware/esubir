...
if ($islem == "havale_cikis_detay_modal_getir") {
    ?>
    <style>
        #banka_havale_cikis_detay_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="banka_havale_cikis_detay_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
    ...
    <script>
        ...
        $("body").off("click", "#cikis_vazgec").on("click", "#cikis_vazgec", function () {
            $("#banka_havale_cikis_detay_modal").modal("hide");
        })
        ...
        $(document).ready(function () {
            ...
            $("#banka_havale_cikis_detay_modal").modal("show");
            ...
            table = $('#banka_havale_list').DataTable({
                ...
                createdRow: function (row) {
                    $(row).addClass("havale_cikis_main_selected");
            ...
            var ilk_tutar = 0;
            $.get("controller/banka_controller/sql.php?islem=havale_cikis_ust_bilgiler", {id: "<?=$_GET["id"]?>"}, function (result) {
                if (result != 2) {
                    var item = JSON.parse(result);
                    let islem_tarihi = item.islem_tarihi;
            ...
            $.get("controller/banka_controller/sql.php?islem=cikis_urunleri_getir_sql", {cikis_id: "<?=$_GET["id"]?>"}, function (result) {
                if (result != 2) {
                    var json = JSON.parse(result);
                    let toplam_tutar = 0;
                    json.forEach(function (item) {
        ...
        $("body").off("click", ".havale_cikis_main_selected").on("click", ".havale_cikis_main_selected", function () {
            var id = $(this).attr("data-id");
            var cari_id = $(this).attr("cari");
            var cari_kodu = $(this).find('td').eq(0).text();
            var cari_adi = $(this).find('td').eq(1).text();
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == 1) {
                        ...
                        let toplam_tutar = 0;
                        $(".havale_cikis_main_selected").each(function () {
