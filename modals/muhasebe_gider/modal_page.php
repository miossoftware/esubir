...
if ($islem == "muhasebe_gider_faturasi_guncelle_modal") {
    ?>
    <style>
        #gider_faturasi_guncelle_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="gider_faturasi_guncelle_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
                ...
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        <div class="ibox-head" style='background-color:#9DB2BF'>
                            <div class="ibox-title" style='color:white; font-weight:bold;'>GİDER FATURASI GÜNCELLE</div>
                        </div>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result == 1) {
                        ...
                        $("#gider_faturasi_guncelle_modal").modal("hide");
                    } else {
                        Swal.fire(
                            "Oops...",
        ...
        $(document).ready(function () {
            $("#gider_faturasi_guncelle_modal").modal("show");
            setTimeout(function () {
            ...
            $.get("controller/gider_controller/sql.php?islem=guncellenecek_gider_bilgilerini_getir_sql", {id: "<?=$_GET["id"]?>"}, function (result) {
                if (result != 2) {
                    var item = JSON.parse(result);
                    let fatura_tarihi = item.fatura_tarihi;
                    fatura_tarihi = fatura_tarihi.split(" ");
                    fatura_tarihi = fatura_tarihi[0];
                    $("#gider_fatura_no").val(item.fatura_no);
                    $("#fatura_tarihi").val(fatura_tarihi);
                    $("#gider_aciklama").val(item.aciklama);
                    ...
                    $(".kdv_toplam_bas").html("");
                    $(".kdv_toplam_bas").html(kdv_tutar + " TL");
                    $(".iskonto_toplam_bas").html("");
                    $(".iskonto_toplam_bas").html(iskonto_tutar + " TL");
                    $(".genel_toplam_bas").html("");
                    $(".genel_toplam_bas").html(genel_toplam + " TL");
                }
            })

            $.get("controller/gider_controller/sql.php?islem=gider_fatura_urunleri_getir_sql", {id: "<?=$_GET["id"]?>"}, function (result) {
                if (result != 2) {
                    ...
                    json.forEach(function (item) {
                        ...
                        muhasebe_gider_faturalari.row.add([item.cari_kodu, item.cari_adi, kdv_yuzde, kdv_tutar, iskonto_yuzde, iskonto_tutar, ara_toplam, genel_toplam, item.aciklama, "<button class='btn btn-danger btn-sm muhasebe_gider_urunu_kesin_sil_button' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"]).draw(false);
                    });
                }
            })
        ...
        $("body").off("click",".muhasebe_gider_urunu_kesin_sil_button").on("click",".muhasebe_gider_urunu_kesin_sil_button",function (){
