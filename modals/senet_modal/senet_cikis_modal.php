...
if ($islem == "cek_senet_cikisi_ekle_main_modal") {
    ?>
    <style>
        #yeni_senet_cikis_olustur_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="yeni_senet_cikis_olustur_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
                ...
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        <div class="ibox-head" style='background-color:#9DB2BF'>
                            <div class="ibox-title" style='color:white; font-weight:bold;'>SENET ÇIKIŞ CİROSU</div>
                        </div>
                        <div class="modal-body">
                            <div class="yeni_cek_cikisi"></div>
                            <div class="yeni_cek_icin_cari"></div>
                            <div class="kendi_senetlerimiz_cikisi"></div>
                            ...
                            <div class="col-12 mx-1 row no-gutteras" style="border: 1px solid #000">
                                <div class="col-6 mt-2">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <select class="custom-select custom-select-sm senet_bankaid" id="banka_id">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-md-3">
                                            <label>Senet Tipi</label>
                                        </div>
                                        <div class="col-md-6">
                                            <select class="custom-select custom-select-sm" id="islem">
                                                <option value="">Seçiniz...</option>
                                                <option value="1">Ciro Edilmiş Senet</option>
                                                <option value="2">Tahsile Verilmiş Senet</option>
                                                <option value="3">Teminata Verilmiş Senet</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card-footer">
                                ...
                                <button class="btn btn-primary btn-sm" id="kendi_senetimizi_olustur_cikis_modal">Kendi
                                    Senetimiz
                                </button>
    ...
    <script>
        ...
        $("body").off("click", "#kendi_senetimizi_olustur_cikis_modal").on("click", "#kendi_senetimizi_olustur_cikis_modal", function () {
            ...
            if (cari_id == "" || cari_id == undefined) {
                ...
                $.get("modals/senet_modal/senet_cikis_modal.php?islem=kendi_ceklerimizi_olustur", {cari_id: cari_id}, function (getModal) {
                    $(".kendi_senetlerimiz_cikisi").html("");
                    $(".kendi_senetlerimiz_cikisi").html(getModal);
                })
            }
        });

        $("body").off("click", ".bizim_seneti_iptal_et_sql").on("click", ".bizim_seneti_iptal_et_sql", function () {
            let id = $(this).attr("data-id");
            let closest = $(this).closest("tr");
            $.ajax({
                url: "controller/senet_controller/sql.php?islem=kendi_senetimizi_iptal_et_sql",
                type: "POST",
                data: {
                    id: id
                },
                success: function (result) {
                    if (result == 1) {
                        ...
                        $("#kendi_senetimizi_olustur_cikis_modal").attr("data-id", "");
                    }
                }
            })
        });

        $.get("controller/senet_controller/sql.php?islem=bankalari_getir_sql", function (result) {
            if (result != 2) {
                ...
                json.forEach(function (item) {
                    ...
                    $("#banka_id").append("" +
                        "<option value='" + item.id + "'>" + item.banka_adi + " (" + sube_adi + ")</option>" +
                        "");
                })
            }
        })
        ...
        $("body").off("click", "#alinan_cek_cikisi_onayla").on("click", "#alinan_cek_cikisi_onayla", function () {
            var arr = [];
            let kendi_cekimiz_mi = $("#kendi_senetimizi_olustur_cikis_modal").attr("data-id");
            $(".cek_senet_cikis_table").each(function () {
                let islem = $("#islem").val();
                if (islem == "1") {
                    islem = "cek_senet_cirosu";
                    let alinan_cekid = $(this).attr("alinan_senetid");
            ...
            let cari_id = $("#cek_senet_cikis_cariid").attr("data-id");
            let banka_id = $(".senet_bankaid").val();
            let islem = $("#islem").val();
            ...
            if (islem == "") {
                ...
                if (islem != 1 && banka_id == "") {
                    Swal.fire(
                        'Uyarı!',
                        'Bu Çıkış Tipinde Banka Boş Kalamaz',
                        'warning'
                    );
                } else {
                    if (kendi_cekimiz_mi == "" || kendi_cekimiz_mi == undefined) {
                        $.ajax({
                            url: "controller/senet_controller/sql.php?islem=senet_cikis_kaydet_sql",
                            type: "POST",
                            data: {
                                cari_id: cari_id,
                                tarih: tarih,
                            ...
                            success: function (result) {
                                if (result == 1) {
                                    $("#yeni_senet_cikis_olustur_modal").modal("hide");
                        ...
                        $.ajax({
                            url: "controller/senet_controller/sql.php?islem=kendi_cekimizin_bilgilerini_guncelle",
                            type: "POST",
                            data: {
                                cari_id: cari_id,
                                tarih: tarih,
        ...
        $("body").off("click", "#cek_senet_vazgec_main").on("click", "#cek_senet_vazgec_main", function () {
            ...
            if (kendi_cekimiz_mi == "" || kendi_cekimiz_mi == undefined) {
                ...
                $.ajax({
                    url: "controller/senet_controller/sql.php?islem=kendi_cekimiz_vazgec_sql",
                    type: "POST",
                    data: {
                        id: kendi_cekimiz_mi
                    },
        ...
        $(document).ready(function () {
            ...
            $("#yeni_senet_cikis_olustur_modal").modal("show");
        ...
        $("body").off("click", "#cek_senet_listesi_cikis_modal_button").on("click", "#cek_senet_listesi_cikis_modal_button", function () {
            ...
            if (cari_id == "" || cari_id == undefined) {
                ...
                $.get("modals/senet_modal/senet_cikis_modal.php?islem=kesilen_giris_senetlerini_getir_list_modal", {cari_id: cari_id}, function (getModal) {
...
if ($islem == "kesilen_giris_senetlerini_getir_list_modal") {
    ?>
    <div class="modal fade" id="cariye_ait_girilen_cekleri_getir"
        ...
        <div class="modal-dialog"
            ...
            <div class="modal-content">
                <div class="modal-header text-white p-2" style="font-weight: bold">CARİYE AİT SENETLER
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.get("controller/senet_controller/sql.php?islem=senetleri_cikis_icin_getir_sql", {cari_id: "<?=$_GET["cari_id"]?>"}, function (result) {
                if (result != 2) {
                    ...
                    json.forEach(function (item) {
                        ...
                        $(new_row).attr("alinan_senetid", item.alinan_senetid);
                    })
                }
            })
        })
        ...
        $("body").off("click", "#secilen_cekler_ve_senetleri_aktar").on("click", "#secilen_cekler_ve_senetleri_aktar", function () {
            ...
            $(".cariye_ait_cekler_ve_senetler_selected").each(function () {
                let id = $(this).attr("alinan_senetid");
            ...
            arr.forEach(function (item) {
                ...
                $(table).attr("alinan_senetid", item.alinan_senetid);
            })

            let vade_gun_fark = toplam_tutar_senet / toplam_ana_tutar_senet;
            vade_gun_fark = parseInt(vade_gun_fark);
            if (isNaN(vade_gun_fark)) {
                vade_gun_fark = 0;
            }
...
if ($islem == "kendi_ceklerimizi_olustur") {
    ?>
    <div class="modal fade" id="kendi_senetlerimizi_ver_modal_main"
         data-bs-keyboard="false" role="dialog">
        <div class="modal-dialog"
             style="width: 60%; max-width: 60%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2" style="font-weight: bold">BİZE AİT SENET VER
                    <button type="button" class="btn-close btn-close-white" id="vazgec_button_kendi_senet"
                            aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="col-12 row">
                        <div class="col-6">
                            <div class="form-group row">
                                ...
                                <div class="col-md-7">
                                    <input type="text" class="form-control form-control-sm" id="bizim_senet_seri_no">
                                </div>
                            </div>
                            <div class="form-group row">
                                <div class="col-4">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-7">
                                    <div class="input-group">
                                        ...
                                        <div class="input-group-append">
                                            <button class="btn btn-warning btn-sm"><i class="fa fa-ellipsis-h"></i>
                                            </button>
                                        </div>
                                    </div>
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-danger btn-sm" id="vazgec_button_kendi_senet"><i class="fa fa-close"></i>
                            Vazgeç
                        </button>
                        <button class="btn btn-success btn-sm" id="kendi_senetlerimize_ekle"><i class="fa fa-check"></i>
                            Kaydet
                        </button>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            $("#kendi_senetlerimizi_ver_modal_main").modal("show");
        })
        ...
        $("body").off("click", "#kendi_senetlerimize_ekle").on("click", "#kendi_senetlerimize_ekle", function () {
            let seri_no = $("#bizim_senet_seri_no").val();
            ...
            let bizim_cekid = $("#kendi_senetimizi_olustur_cikis_modal").attr("data-id");
            if (banka_id == "" || banka_id == undefined) {
                ...
                $.ajax({
                    url: "controller/senet_controller/sql.php?islem=bize_ait_cek_olustur_sql",
                    type: "POST",
                    data: {
                        seri_no: seri_no,
                        cari_id: cari_id,
                    ...
                    success: function (result) {
                        if (result != 2) {
                            ...
                            cek_table_cikis_main.clear().draw(false);
                            json.forEach(function (item) {
                                $("#kendi_senetimizi_olustur_cikis_modal").attr("data-id", item.bizim_senetid);
                                ...
                                $("#kendi_senetlerimizi_ver_modal_main").modal("hide");
                            });
                            $("#islem").val("1");
        ...
        $("body").off("click", "#vazgec_button_kendi_senet").on("click", "#vazgec_button_kendi_senet", function () {
            $("#kendi_senetlerimizi_ver_modal_main").modal("hide");
