...
if ($islem == "senet_odeme_al_main") {
    ?>
    <style>
        #senet_odeme_ali_main_modal {
            overflow-y: scroll;
        }
    </style>
    <div class="modal fade" id="senet_odeme_ali_main_modal" data-backdrop="static" data-bs-keyboard="false"
         role="dialog">
        <div class="modal-dialog" style="width: 75%; max-width: 75%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2">
                    <button type="button" class="btn-close btn-close-white" id="senet_odeme_main"
                            aria-label="Close"></button>
                </div>
                <div class="page-content fade-in-up">
                    <div class="ibox">
                        <div class="ibox-head" style='background-color:#9DB2BF'>
                            <div class="ibox-title" style='color:white; font-weight:bold;'>SENET ÖDEME (ÇIKIŞ)
                            </div>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="card-footer">
                                <button class="btn btn-danger btn-sm" id="senet_odeme_main"><i
                                            class="fa fa-close"></i> Vazgeç
                                </button>
                                <button class="btn btn-success btn-sm" id="secilen_senetleri_ode"><i
                                            class="fa fa-check"></i> Kaydet
                                </button>
                                <button class="btn btn-info btn-sm" id="senet_odenecekleri_getir"><i
    ...
    <script>
        ...
        $("body").off("click", "#secilen_senetleri_ode").on("click", "#secilen_senetleri_ode", function () {
            var gidecek_arr = [];
            var firstTrDataId = senet_odeme_table.row(':eq(0)').data()[6];
            let varmi = false;
            ...
            if (varmi == false) {
                ...
                $.ajax({
                    url: "controller/senet_controller/sql.php?islem=odemeyi_gerceklestir_sql",
                    type: "POST",
                    data: {
                        tarih: tarih,
                        belge_no: belge_no,
                    ...
                    success: function (result) {
                        if (result == 1) {
                            ...
                            $("#senet_odeme_ali_main_modal").modal("hide");
        ...
        $("body").off("click", "#senet_odenecekleri_getir").on("click", "#senet_odenecekleri_getir", function () {
            $.get("modals/senet_odeme_al_modal/modal.php?islem=cek_senet_odenecekleri_getir", function (getModal) {
                $(".odenecekleri_getir_div").html("");
                $(".odenecekleri_getir_div").html(getModal);
            });
        });

        var senet_odeme_table = "";
        $(document).ready(function () {
            $("#senet_odeme_ali_main_modal").modal("show");
            ...
            senet_odeme_table = $('#tahsile_verilen_cek_senet_tahsili_table').DataTable({
                scrollY: '30vh',
                scrollX: true,
                "info": false,
                bAutoWidth: false,
        ...
        $("body").off("click",".odeme_alinacak_senet_sil_main_button").on("click",".odeme_alinacak_senet_sil_main_button",function (){
            var row = $(this).closest('tr');
            senet_odeme_table.row(row).remove().draw(false);
        });

        $("body").off("click", "#senet_odeme_main").on("click", "#senet_odeme_main", function () {
            $("#senet_odeme_ali_main_modal").modal("hide");
        });
...
if ($islem == "cek_senet_odenecekleri_getir") {
    ?>
    <div class="modal fade" id="kendi_senetlerimizi_getir_main_modal"
         data-bs-keyboard="false" role="dialog">
        <div class="modal-dialog"
             style="width: 55%; max-width: 55%;">
            <div class="modal-content">
                <div class="modal-header text-white p-2" style="font-weight: bold">Kendi Senetlerimiz
    ...
    <script>
        $(document).ready(function () {
            $("#kendi_senetlerimizi_getir_main_modal").modal("show");
            setTimeout(function () {
            ...
            var senet_odenecekler_list_main = $('#tahsile_verilen_cek_senetler_list').DataTable({
                scrollY: '30vh',
                scrollX: true,
                "info": false,
                bAutoWidth: false,
            ...
            $.get("controller/senet_controller/sql.php?islem=bizim_senetleri_getir_sql", function (result) {
                if (result != 2) {
                    ...
                    json.forEach(function (item) {
                        ...
                        let table = senet_odenecekler_list_main.row.add(["<input type='checkbox' class='col-9 secilen_tahsillikler' data-id='" + item.id + "'>", item.seri_no, vade_tarih, "BİZ", item.cari_adi, tutar, item.keside_yeri, item.banka_adi, item.sube_adi, item.hesap_no, item.ozel_kod]).draw(false).node();
        ...
        $("body").off("click", "#secilen_tahsilleri_aktar").on("click", "#secilen_tahsilleri_aktar", function () {
            senet_odeme_table.clear().draw(false);
            $(".tahsile_verilen_selected").each(function () {
                if ($(this).hasClass("tahsil_secildi")) {
                    ...
                    let secilenler = senet_odeme_table.row.add([seri_no, vade_tarih, asil_borclu, ciro_edilmis, tutar, keside_yeri, banka_adi, sube_adi, hesap_no, ozel_kod, "ÖDENDİ", "BİZİM", "<button class='btn btn-danger btn-sm odeme_alinacak_senet_sil_main_button'><i class='fa fa-trash'></i></button>"]).draw(false).node();
            ...
            $("#kendi_senetlerimizi_getir_main_modal").modal("hide");
        });
