<div class="ibox-container">
    <div class="ibox mt-5">
        <div class="ibox-head">
            <div class="ibox-title" style=' font-weight:bold;'>BİNEK ARAÇ HGS GİDER GİRİŞİ</div>
        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("controller/arac_controller/sql.php?islem=hgs_gider_fislerini_getir_sql", function (response) {
            if (response != 2) {
                ...
                json.forEach(function (item) {
                    ...
                    let newRow = {
                        ...
                        button: "<button class='btn btn-sm binek_hgs_gider_fisi_guncelle_main_button' data-id='" + item.id + "' style='background-color: #F6FA70'><i class='fa fa-refresh'></i></button> <button class='btn btn-danger btn-sm binek_hgs_gider_fisi_sil_main_list_button' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>",
                    };
    ...
    $("body").off("click", "#binek_arac_hgs_gider_ekle_button").on("click", "#binek_arac_hgs_gider_ekle_button", function () {
        $.get("modals/arac_modal/hgs_gider_giris.php?islem=hgs_gider_girisi_modal_getir", function (getModal) {
            $(".getModals").html(getModal);
        })
    });

    $("body").off("click", ".binek_hgs_gider_fisi_guncelle_main_button").on("click", ".binek_hgs_gider_fisi_guncelle_main_button", function () {
        let id = $(this).attr("data-id");
        $.get("modals/arac_modal/hgs_gider_fisi_page.php?islem=hgs_gider_fisi_detay_modal_getir", {id: id}, function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });

    $("body").off("click", ".binek_hgs_gider_fisi_sil_main_list_button").on("click", ".binek_hgs_gider_fisi_sil_main_list_button", function () {
        ...
        if (cari_kodu == "" || cari_kodu == undefined) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "controller/arac_controller/sql.php?islem=hgs_gider_fisi_tum_sil_sql",
                        type: "POST",
                        data: {
                            id: cari_kodu,
