...
<div class="ibox-container">
    ...
    <div class="ibox mt-5">
        <div class="ibox-head">
            <div class="ibox-title" style=' font-weight:bold;'>CARİ HESAP EKSTRESİ</div>
        </div>
        <div class="col-12 row no-gutters mt-3">
            <div class="col-md-1">
                <div class="input-group mb-3">
                    <input type="text" class="form-control form-control-sm secilen_cari"
                           aria-describedby="basic-addon1" id="cari_id_filter" placeholder="Cari Kodu">
            ...
            <div class="col-md-3 mx-1">
                <input type="text" style="background-color: #e08e0b; font-weight: bold;color: white !important;"
                       class="form-control form-control-sm cari_adi_getir" disabled value="Cari Adı">
            </div>
        ...
        <nav class="nav nav-pills flex-column flex-sm-row">
            <a class="flex-sm-fill text-sm-center nav-link active cari_page_color" aria-current="page"
               id="hesap_ekstre_genel" style="font-weight: bold">HESAP EKSTRESİ</a>
            <a class="flex-sm-fill text-sm-center nav-link cari_page_color" style="font-weight: bold"
               id="cari_alis_irsaliyeleri">ALIŞ İRSALİYELERİ</a>
            <a class="flex-sm-fill text-sm-center nav-link cari_page_color" style="font-weight: bold"
               id="cari_satis_irsaliyeleri">SATIŞ İRSALİYELERİ</a>
        </nav>
...
<script>
    ...
    $("body").off("keyup", "#cari_id_filter").on("keyup", "#cari_id_filter", function () {
        ...
        $.get("konteyner/controller/irsaliye_controller/sql.php?islem=cari_kodu_bilgileri_getir_sql", {cari_kodu: val}, function (result) {
            if (result != 2) {
                ...
                $("#cari_id_filter").val(item.cari_kodu);
                $(".cari_adi_getir").val((item.cari_adi).toUpperCase());
            } else {
    ...
    $("body").off("click", "#cari_getir_modal").on("click", "#cari_getir_modal", function () {
        $.get("modals/ekstre_modal.php?islem=cariler_listesi_getir_modal", function (getModal) {
    ...
    $("body").off("click", "#ekstre_filtrele").on("click", "#ekstre_filtrele", function () {
        ...
        $.get("controller/ekstre_controller/sql.php?islem=cari_hesap_ekstre_getir", {
            cari_id: cari_id,
            bas_tarih: bas_tarih,
