...
<div class="ibox mt-5">
    <div class="ibox-head">
        <div class="ibox-title" style=' font-weight:bold;'>ÇEK ÖDEME (ÇIKIŞ)</div>
    </div>
    <div class="col-12 row">
        <div class="mx-2">
            <button class="btn btn-success btn-sm" id="cek_senet_ode_button_main"><i class="fa fa-plus"></i> Çek
                Öde
            </button>
        ...
        <div class="col-12 row mt-3">
            <table class="table table-sm table-bordered w-100  nowrap" style="cursor:pointer;font-size: 13px;"
                ...
                <th></th>
                <th></th>
                <th style="text-align: right" class="odenene_cek_tot">0,00</th>
                <th></th>
                <th></th>
                </tfoot>
            </table>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("controller/cek_senet_tahsil_odeme_controller/sql.php?islem=odenen_cekleri_getir_sql", function (result) {
            if (result != 2) {
                var json = JSON.parse(result);
                var basilacak_arr = [];
                let odenene_cek_tot = 0;
                json.forEach(function (item) {
                    ...
                    let newRow = {
                        ...
                        'button': "<button class='btn btn-danger btn-sm cek_odeme_iptal_et_main_button' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                    };
                    basilacak_arr.push(newRow);
                });
                $(".odenene_cek_tot").html(odenene_cek_tot.toLocaleString("tr-TR", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }))
    ...
    $("body").off("click", "#cek_senet_ode_button_main").on("click", "#cek_senet_ode_button_main", function () {
        $.get("modals/cek_senet_odeme_modal/modal.php?islem=cek_senet_odeme_al", function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    })

    $("body").off("click", ".cek_odeme_iptal_et_main_button").on("click", ".cek_odeme_iptal_et_main_button", function () {
        ...
        if (id == "" || id == undefined) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "controller/cek_senet_tahsil_odeme_controller/sql.php?islem=cek_senet_odemeyi_iptal_et_sql",
                        type: "POST",
                        data: {
                            id: id,
