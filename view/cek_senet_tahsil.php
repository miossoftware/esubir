...
<div class="ibox mt-5">
    <div class="ibox-head">
        <div class="ibox-title" style=' font-weight:bold;'>ÇEK TAHSİLAT (GİRİŞ)</div>
    </div>
    <div class="col-12 row">
        <div class="mx-2">
            <div class="btn-group" role="group">
                <button id="btnGroupVerticalDrop1" type="button"
                    ...
                    Çekler Senetler Tahsili <i class="fa fa-angle-down arrow"></i>
                </button>
                <div class="dropdown-menu">
                    <a class="dropdown-item" id="tahsile_verilmis_cek_senet_tahsili" href="#">Tahsile Verilmiş Çek Senet
                        Tahsili</a>
                    <a class="dropdown-item" id="teminata_verimis_cek_senet_tahsili" href="#">Teminata Verilmiş Çek
                        Senet Tahsili</a>
                    <a class="dropdown-item" id="elden_cek_senet_tahsili" href="#">Elden Çek Senet Tahsili</a>
                </div>
            </div>
        ...
        <div class="col-12 row mt-3">
            <table class="table table-sm table-bordered w-100  nowrap" style="cursor:pointer;font-size: 13px;"
                ...
                <th></th>
                <th></th>
                <th style="text-align: right" class="tahsil_tot">0,00</th>
                <th></th>
                <th></th>
                </tfoot>
            </table>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var table = $('#cek_senet_tahsil_table').DataTable({
            ...
            buttons: [
                {
                    extend: 'excel',
                    title: "ÇEK SENET TAHSİL LİSTESİ",
        ...
        $.get("controller/cek_senet_tahsil_odeme_controller/sql.php?islem=tum_tahsilatlari_getir_sql", function (result) {
            if (result != 2) {
                ...
                json.forEach(function (item) {
                    ...
                    if (item.banka_adi == "" || item.banka_adi == null) {
                        let newRow = {
                            ...
                            "button": "<button class='btn btn-danger btn-sm elden_cek_tahsil_sil_button_main' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                        };
                        ...
                        let newRow = {
                            ...
                            "tutar": tutar,
                            "doviz_tur": item.doviz_tur,
                            "button": "<button class='btn btn-danger btn-sm tahsili_sil_button_main' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                        };
                        bastirilacak_arr.push(newRow);
                    }

                });
                $(".tahsil_tot").html(tahsil_tot.toLocaleString("tr-TR", {
                    maximumFractionDigits: 2,
                    minimumFractionDigits: 2
                }))
    ...
    $("body").off("click", ".tahsili_sil_button_main").on("click", ".tahsili_sil_button_main", function () {
        ...
        if (id == "" || id == undefined) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "controller/cek_senet_tahsil_odeme_controller/sql.php?islem=tahsil_edilen_ceki_sil_sql",
                        type: "POST",
                        data: {
                            id: id,
    ...
    $("body").off("click", ".elden_cek_tahsil_sil_button_main").on("click", ".elden_cek_tahsil_sil_button_main", function () {
        let id = $(this).attr("data-id");
        if (id == "" || id == undefined) {
            Swal.fire(
                'Uyarı!',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const delete_detail = result.value;
                    $.ajax({
                        url: "controller/cek_senet_tahsil_odeme_controller/sql.php?islem=elden_cek_tahsil_sil_sql",
                        type: "POST",
                        data: {
                            id: id,
                            delete_detail: delete_detail
    ...
    $("body").off("click", "#tahsile_verilmis_cek_senet_tahsili").on("click", "#tahsile_verilmis_cek_senet_tahsili", function () {
        $.get("modals/cek_senet_tahsil_modal/modal.php?islem=cek_senet_tahsil_et_modal", function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });
    $("body").off("click", "#teminata_verimis_cek_senet_tahsili").on("click", "#teminata_verimis_cek_senet_tahsili", function () {
        $.get("modals/cek_senet_tahsil_modal/teminata_verimis_cek_senet_tahsili.php?islem=cek_senet_tahsil_et_modal", function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });

    $("body").off("click", "#elden_cek_senet_tahsili").on("click", "#elden_cek_senet_tahsili", function () {
        $.get("modals/cek_senet_tahsil_modal/elden_cek_senet_tahsili.php?islem=elden_cek_senet_tahsili_getir_modal", function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });
