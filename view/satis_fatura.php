...
<div class="ibox mt-5">
    <div class="ibox-head">
        <div class="ibox-title" style=' font-weight:bold;'>SATIŞ FATURA</div>
    </div>
    <div class="col-12 row tetikle">
        <div class="col-12 row mx-1 mt-3">
            ...
            <div class="col-md-2 row mx-1">
                <select class="custom-select custom-select-sm" id="doviz_tur_satis_filter">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $.get("controller/satis_controller/sql.php?islem=faturalari_getir_sql", function (result) {
            if (result != 2) {
                ...
                json.forEach(function (item) {
                    ...
                    let karakter_sayisi = aciklama.length;
                    if (karakter_sayisi > 15) {
                        aciklama = aciklama.substring(0, 15)+"...";
                    }

                    let disabled = "";
                    if ("geldigi_yer" in item){
                        disabled = "disabled";
                    }
                    ...
                    let newRow = {
                        ...
                        button: "<button style='background-color: #F6FA70' class='btn btn-sm fatura_goruntule_button_main' "+disabled+" data-id='" + item.id + "'><i class='fa fa-eye'></i></button> <button class='btn btn-danger btn-sm satis_faturasini_sil_main_buton' "+disabled+" data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                    }
                    basilacak_arr2.push(newRow);
                });
        ...
        $("body").off("click", "#alis_fatura_filtrele").on("click", "#alis_fatura_filtrele", function () {
            ...
            var doviz_tur = $("#doviz_tur_satis_filter").val();
            ...
            if (bas_tarih > bit_tarih) {
                ...
                $.get("controller/satis_controller/sql.php?islem=alis_faturalari_filtrele_sql", {
                    bas_tarih: bas_tarih,
                    bit_tarih: bit_tarih,
                    bas_fiyat: bas_fiyat,
                    bit_fiyat: bit_fiyat,
                    ...
                    if (result != 2) {
                        ...
                        json.forEach(function (item) {
                            ...
                            let newRow = {
                                ...
                                kdv_toplam: kdv_toplam + " " + item.doviz_tur,
                                genel_toplam: genel_toplam + " " + item.doviz_tur,
                                button: "<button style='background-color: #F6FA70' class='btn btn-sm fatura_goruntule_button_main' data-id='" + item.id + "'><i class='fa fa-eye'></i></button> <button class='btn btn-danger btn-sm satis_faturasini_sil_main_buton' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                            }
                            basilacak_arr2.push(newRow);
                        });
                        table.rows.add(basilacak_arr2).draw(false);
    ...
    $("body").off("click", "#alis_fatura_olustur_main").on("click", "#alis_fatura_olustur_main", function () {
        $.get("modals/satis_modal/modal.php?islem=alis_faturasi_ekle", function (getModal) {
            $(".getModals").html("");
    ...
    $("body").off("click", ".fatura_goruntule_button_main").on("click", ".fatura_goruntule_button_main", function () {
        ...
        $.get("modals/satis_modal/modal_page.php?islem=faturayi_goruntule_ve_guncelle", {id: id}, function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });
    $("body").off("click", ".satis_faturasini_sil_main_buton").on("click", ".satis_faturasini_sil_main_buton", function () {
        var id = $(this).attr("data-id");
        if (id == "" || id == undefined) {
            Swal.fire(
                'Uyarı!',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "controller/satis_controller/sql.php?islem=fatura_sil_sql",
                        type: "POST",
                        data: {
                            id: id,
