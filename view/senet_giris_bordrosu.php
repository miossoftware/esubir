...
<div class="ibox mt-5">
    <div class="ibox-head">
        <div class="ibox-title" style=' font-weight:bold;'>SENET GİRİŞ BORDROSU</div>
    </div>
    <div class="col-12 row">
        <div class="mx-2">
            <button class="btn btn-success btn-sm" id="alinan_senetler_girisi"><i class="fa fa-plus"></i> Alınan
                Senetler Girişi
            </button>
...
<script>
    ...
    $("body").off("click", ".senet_giris_guncelle_firs_button").on("click", ".senet_giris_guncelle_firs_button", function () {
        let id = $(this).attr("data-id");
        $.get("modals/senet_modal/modal_page.php?islem=senet_guncelle_modal_getir", {id: id}, function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    });
    ...
    $(document).ready(function () {
        ...
        $("body").off("click", "#cek_senet_giris_hazirla_buton").on("click", "#cek_senet_giris_hazirla_buton", function () {
            ...
            $.get("controller/senet_controller/sql.php?islem=tum_cek_ve_senetleri_getir_sql",
                {
                    bas_tarih: bas_tarih,
                    bit_tarih: bit_tarih
                }
                    ...
                    if (result != 2) {
                        ...
                        json.forEach(function (item) {
                            ...
                            let newRecord = {
                                ...
                                'button': "<button class='btn btn-sm senet_giris_guncelle_firs_button' data-id='" + item.id + "' style='background-color: #F6FA70'><i class='fa fa-refresh'></i></button> <button class='btn btn-danger btn-sm ana_sayfadan_senet_sil' data-id='" + item.id + "'><i class='fa fa-trash'></i></button>"
                            };
        ...
        $.get("controller/senet_controller/sql.php?islem=tum_cek_ve_senetleri_getir_sql", function (result) {
            if (result != 2) {
                var json = JSON.parse(result);
                let basilacak_arr = [];
                json.forEach(function (item) {
    ...
    $("body").off("click", "#alinan_senetler_girisi").on("click", "#alinan_senetler_girisi", function () {
        $.get("modals/senet_modal/modal.php?islem=senet_giris_modal_getir_sql", function (getModal) {
            $(".getModals").html("");
            $(".getModals").html(getModal);
        })
    })

    $("body").off("click", ".ana_sayfadan_senet_sil").on("click", ".ana_sayfadan_senet_sil", function () {
        let id = $(this).attr("data-id");
        if (id == "" || id == undefined) {
            Swal.fire(
                'Uyarı!',
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "controller/senet_controller/sql.php?islem=cek_senet_sil_sql",
                        type: "POST",
                        data: {
                            id: id,
